...
export namespace Newsletter {
    export function newsletterData(newsletter: Newsletter, showUnsubscribe: boolean, linkService: LinkService<InternalPathKey>): NewsletterData {
        const departmentsItems: Element<NewsletterData['content']>['items'] | null = values(newsletter.departments).some(departments => departments !== null && departments.length !== 0) ? compactMap(entries(newsletter.departments), ({ key, value }) =>(value === null ? null : {
            ...
            link: linkService.absoluteLink(Department.link[key]),
        ...
        return {
            ...
            unsubscribeLink: showUnsubscribe ? linkService.absoluteLink(linkConfig["newsletter/unsubscribe/:id"].replaceAll(':id', '%newsletter-subscriber-id%')) : null,
            websiteLink: {
                title: 'www.svkleinsendelbach-website.web.app',
                link: linkService.absoluteLink('')
            },
            notCorrectShownLink: linkService.absoluteLink(linkConfig["newsletter/:id"].replaceAll(':id', newsletter.id)),
            content: [
    ...
    export namespace Department {
        ...
        export const link: Record<Department, string> = {
            'football-adults/general': linkConfig["football-adults"],
            'football-adults/first-team': linkConfig["football-adults/first-team"],
            'football-adults/second-team': linkConfig["football-adults/second-team"],
            'football-youth/big-field': linkConfig["football-youth"],
            'football-youth/small-field': linkConfig["football-youth"],
            'gymnastics': linkConfig.gymnastics,
            'dancing': linkConfig.dancing
        };
    }
